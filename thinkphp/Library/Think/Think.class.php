...
class Think {
    ...
    static public function start() {
      ...
      // 设定错误和异常处理2016.10.1注释掉了
      register_shutdown_function('Think\Think::fatalError');
      set_error_handler('Think\Think::appError');
      set_exception_handler('Think\Think::appException');

      // 初始化文件存储方式
      // Storage是操作common~runtime.php文件的（引入、写入）
      ...
      if(!APP_DEBUG && Storage::has($runtimefile)){
        //生产模式引入common~runtime.php文件
          ...
          if(Storage::has($runtimefile))
              Storage::unlink($runtimefile);//调试模试删除runtime.php文件
          $content =  '';
          // 读取应用模式，读取就是ThinkPHP/Conf/Mode/common.php
    ...
    public static function autoload($class) {
        ...
        if(isset(self::$_map[$class])) {
           ...
           //strstr(Think\Verify,'\\')----Verify
            //strstr(Think\Verify,'\\',true)-----Think
            //$name = Think;
